...
def operate_on_one_job(func, output_dir):
    ...
    if func == 'abort':
        ...
        # please install azcopy version 10 or above to use this function.
        sa = StorageAccount(aml_job_cfg['datastore'])
        cmd = 'azcopy cp https://{}.blob.core.windows.net/{}/{}?{} ' \
              '--destination {} --recursive'.format(
                sa.account_name, sa.container_name, output_dir, sa.account_key,
                output_dir)
        print_run_cmd(cmd)
    elif func == 'remove':
